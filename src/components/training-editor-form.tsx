...
export function TrainingEditorForm({ action, values = {}, saved = false }: {
  ...
  return <form action={action} className="training-editor-form">
    ...
    <header><div><Link href="/admin">← 管理後台</Link><p className="eyebrow">TRAINING EDITOR</p><h1>{values.id ? "編輯訓練" : "新增訓練"}</h1></div><div>{saved && <span className="saved-badge">已儲存</span>}<label className="publish-check"><input name="published" type="checkbox" defaultChecked={values.published}/> 發布</label><button type="submit" className="training-save-button">{values.id ? "儲存訓練" : "建立訓練"} →</button></div></header>
    <section>
      ...
      <label>季節<StyledSelect name="season" defaultValue={values.season ?? "WINTER"} ariaLabel="季節" options={[{ value: "WINTER", label: "寒訓" }, { value: "SUMMER", label: "暑訓" }]}/></label>
