...
function SubmitButton() {
  ...
  return <button disabled={pending}>{pending ? "建立帳號中…" : "註冊成員帳號"}<span>→</span></button>;
}
...
export function RegisterForm() {
  ...
  return <form className="login-form register-form" action={action}>
    <div><span className="mono">LABCAMP / REGISTER</span><Link href="/login" prefetch={false}>← 返回登入</Link></div>
    <label>姓名<input name="name" type="text" minLength={2} maxLength={50} autoComplete="name" required placeholder="研究室成員姓名"/></label>
    <label>電子信箱<input name="email" type="email" autoComplete="email" required placeholder="name@lab.edu.tw"/></label>
