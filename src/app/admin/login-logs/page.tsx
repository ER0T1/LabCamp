...
export default async function LoginLogsPage({ searchParams }: { searchParams: Promise<LoginLogParams> }) {
  ...
  return <div className="page-shell inner-page admin-page">
    ...
    <div className="login-log-stats" aria-label="登入日誌摘要">
      <span><b>{eventCounts.reduce((sum, item) => sum + item._count._all, 0)}</b><small>全部事件</small></span>
      <span><b>{countByEvent.get("LOGIN_SUCCESS") ?? 0}</b><small>登入成功</small></span>
      <span><b>{countByEvent.get("LOGIN_FAILED") ?? 0}</b><small>登入失敗</small></span>
      <span><b>{countByEvent.get("LOGOUT") ?? 0}</b><small>登出</small></span>
    </div>
    ...
    <section className="admin-table login-log-management">
      ...
      <form className="login-log-filters" action="/admin/login-logs" method="get">
        ...
        <label>事件<select name="event" defaultValue={event ?? ""}><option value="">全部事件</option><option value="LOGIN_SUCCESS">登入成功</option><option value="LOGIN_FAILED">登入失敗</option><option value="LOGOUT">登出</option></select></label>
