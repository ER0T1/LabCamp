  ...
  return <div className="page-shell inner-page admin-page">
    ...
    <section className="admin-table">
      ...
      {courses.map(course => <div className="admin-row" key={course.id}>
        ...
        <div className="course-tags admin-course-tags">{course.tags.map(item => <span key={item.tagId}>{item.tag.name}</span>)}{course.tags.length === 0 && <span>UNTAGGED</span>}</div><time>{course.updatedAt.toLocaleDateString("zh-TW")}</time><div className="course-row-actions"><Link className="admin-row-action" href={`/admin/courses/${course.id}/edit`}>編輯課程</Link>{session.user.role === "ADMIN" && <DeleteForm compact action={deleteCourse.bind(null, course.id)} label="刪除課程" confirmMessage={`確定要永久刪除「${course.title}」嗎？共用附件會保留給其他課程，其餘內容無法復原。`}/>}</div>
